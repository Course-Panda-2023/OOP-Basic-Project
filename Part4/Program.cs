...
static void Part4()
{
    ...
    {
        //band.PrintBandParticipants();
        band.SelectSongByBandParticipants(CompetitionSongList);
    }
    //Console.WriteLine(competitionCompetitors.Count);
    int soloist_num = random.Next(1, 10);
    int stringLength = random.Next(3, 12);
    for (int i = 0;i < soloist_num; i++)
    {
        string randomName = Helper.RandomString(stringLength);
